...
function SearchVehicles() {
    ...
    $.ajax({
        url: 'http://localhost:56259/api/inventoryapi/SearchUsedVehicles',
        type: 'post',
        dataType: 'json',
        ...
        success: function (data) {
            $.each(data, function (index, item) {
                ...
                row += '<a href="http://localhost:56259/Inventory/Details/' + item.VehicleId + '" class = "btn btn-dark">Details</a>';
                row += '</div>';
                row += '</div>';
                $('#results').append(row);
            });
